...
class CrawlerManager:
    ...
    def handle_shutdown(self, signum, frame):
        ...
        if hasattr(self, 'stats_tracker'):
            self.stats_tracker.generate_final_summary()
    ...
    def run(self):
        ...
        while total_pages_processed < total_pages:
            ...
            # Run the spider batch and get the output directory
            batch_output_dir = self.run_spider_batch(current_offset, actual_pages_this_batch)
            
            if batch_output_dir and os.path.exists(batch_output_dir):
                # Update global stats with data from this batch
                self.stats_tracker.update_from_batch(batch_output_dir)
    ...
    def run_spider_batch(self, start_offset, pages_per_batch):
        """Run the spider for a specific batch of pages and return the output directory"""
        ...
        try:
            ...
            return batch_output_dir  # Return the output directory for stats collection
...
def main():
    ...
    try:
        ...
        logger.info("Starting crawler manager script")
        
        # Check if we're resuming a previous run
        global_stats_file = os.path.join(config["OUTPUT_DIR"], 'global_stats.json')
        if os.path.exists(global_stats_file):
            logger.info(f"Found existing global stats file at {global_stats_file}")
            logger.info("Will resume previous run and append new statistics")
        else:
            logger.info("Starting fresh crawl with new global statistics")
        
        # Create and run the crawler manager
        manager = CrawlerManager(config)
        manager.run()
        
        # Generate a final summary report with all statistics
        final_summary = manager.stats_tracker.generate_final_summary()
        if final_summary:
            logger.info(f"Final crawl summary:")
            logger.info(f"Total pages processed: {final_summary['total_pages']}")
            logger.info(f"Total products found: {final_summary['total_products']}")
            logger.info(f"Matched products: {final_summary['matched_products']} ({final_summary['match_percentage']:.2f}%)")
            logger.info(f"Total batches run: {final_summary['total_batches']}")
        
        logger.info("Crawler manager completed successfully")
        
    except KeyboardInterrupt:
        if LOGGERS:
            LOGGERS['manager'].info("Interrupted by user. Exiting gracefully.")
            # Try to generate a summary even on interrupt
            if 'manager' in locals() and hasattr(manager, 'stats_tracker'):
                manager.stats_tracker.generate_final_summary()
        else:
