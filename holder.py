    ...
    def run_spider_batch(self, batch_number, start_offset, pages_per_batch, batch_dir):
        ...
        try:
            ...
            cmd = [
                ...
                "-s", "LOG_LEVEL=INFO",
                # Include all performance-related settings from SPIDER_SETTINGS
                "-s", f"DOWNLOAD_DELAY={SPIDER_SETTINGS.get('DOWNLOAD_DELAY', 0)}",
                "-s", f"CONCURRENT_REQUESTS={SPIDER_SETTINGS.get('CONCURRENT_REQUESTS', 64)}",
                "-s", f"CONCURRENT_REQUESTS_PER_DOMAIN={SPIDER_SETTINGS.get('CONCURRENT_REQUESTS_PER_DOMAIN', 64)}",
                "-s", f"AUTOTHROTTLE_ENABLED={SPIDER_SETTINGS.get('AUTOTHROTTLE_ENABLED', 'False')}",
                "-s", f"HTTPCACHE_ENABLED={SPIDER_SETTINGS.get('HTTPCACHE_ENABLED', 'True')}",
                "-s", f"HTTPCACHE_EXPIRATION_SECS={SPIDER_SETTINGS.get('HTTPCACHE_EXPIRATION_SECS', 0)}",
                "-s", f"ROBOTSTXT_OBEY={SPIDER_SETTINGS.get('ROBOTSTXT_OBEY', 'False')}",
                "-s", f"COOKIES_ENABLED={SPIDER_SETTINGS.get('COOKIES_ENABLED', 'False')}",
                "-s", f"REDIRECT_ENABLED={SPIDER_SETTINGS.get('REDIRECT_ENABLED', 'False')}"
            ]
